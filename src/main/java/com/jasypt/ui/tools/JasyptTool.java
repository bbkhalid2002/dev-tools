...
import com.jasypt.ui.JasyptEncryptor;
import javax.swing.*;
...
/**
 * Jasypt Encryption/Decryption tool - refactored from original JasyptGUI.
 */
public class JasyptTool extends JPanel {
	...
	private void initializeUI() {
		...
		// Configuration panel with buttons
	...
	private void performEncryption() {
		try {
			...
			JasyptEncryptor encryptor = new JasyptEncryptor(algorithm, password, salt);
	...
	private void performDecryption() {
		try {
			...
			// Check if input looks like encrypted text
			...
			if (ex.getMessage().contains("base64") || ex.getMessage().contains("byte")) {
